"""
Harvester Adapter.
Interfaces with Harvester (Job Scraping #1) multi-board scrapers
(RemoteOK, Indeed, Wellfound, Naukri) to discover job opportunities.
Translates Harvester's 7-field canonical schema into PostingRef models.
"""
...
class HarvesterAdapter(AgentAdapter):
    """Adapter for executing live or cached job scraping via Harvester / Gleaner."""

    def __init__(
        self,
        harvester_dir: Optional[str] = None,
        default_boards: Optional[List[str]] = None,
        allow_fallback: bool = True,
    ):
        self.harvester_dir = Path(harvester_dir or config.HARVESTER_DIR)
        self.default_boards = default_boards or [
            b.strip().lower()
            for b in config.HARVESTER_DEFAULT_BOARDS.split(",")
            if b.strip()
        ]
    ...
    def _setup_path(self) -> None:
        ...
        if self.harvester_dir.exists() and str(self.harvester_dir) not in sys.path:
            sys.path.insert(0, str(self.harvester_dir))
    ...
    def fetch_jobs(
        ...
        """
        Query specified Harvester board adapters and convert results to PostingRef objects.
        """
        ...
        try:
            ...
            for b_name in target_boards:
                ...
                try:
                    ...
                    logger.warning("Harvester board scraper '%s' failed: %s", b_name, board_err)

                if len(discovered_postings) >= limit:
                    break

        except Exception as import_err:
            logger.warning("Could not import Harvester boards: %s", import_err)
    ...
    def invoke(self, state_dict: Dict[str, Any]) -> AgentResult:
        """
        Execute Harvester discovery or validate pre-supplied posting.
        """
        ...
        try:
            ...
            role = state_dict.get("role") or config.HARVESTER_DEFAULT_ROLE
            location = state_dict.get("location") or config.HARVESTER_DEFAULT_LOCATION
            ...
            if not postings:
                return AgentResult(
                    success=False,
                    error=f"Harvester found 0 postings matching role='{role}' in location='{location}' (EC-03).",
                    latency_ms=(time.time() - start_time) * 1000,
                )
            ...
            return AgentResult(
                success=False,
                error=f"HarvesterAdapter unexpected error: {str(e)}",
                latency_ms=(time.time() - start_time) * 1000,
            )

    def health_check(self) -> bool:
        return self.harvester_dir.exists()
