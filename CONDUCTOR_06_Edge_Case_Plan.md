...
| EC-03 | Upstream failure | Harvester returns zero postings for a run | High (expected, not exceptional) | Low | Not an error — the graph completes normally with an empty Tier-1 loop; this is the routine "no new opportunities today" path, distinct from EC-01 |
...
| EC-06 | Duplicate / re-entrancy | The same posting is discovered twice across two Harvester runs | Medium | Medium | Deduplication check against the persisted run log (CND-IMPL Task 2.4) before entering the Tier-1 loop a second time |
